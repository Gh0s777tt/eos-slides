<!--
eos-slides merge request. The checklist is the Definition of Done from the E-OS
...
- [ ] `cargo test --locked`
- [ ] `cargo run --locked -- --selftest` prints `EOS-SLIDES-SELFTEST-OK`
- [ ] `cargo run --locked --features host-backend -- --check-backend` prints `EOS-SLIDES-BACKEND-OK`
- [ ] `cargo deny check`
...
- [ ] **Pins & mirrors** — pushed to **GitLab and GitHub**, then `repos.toml` + `recipes/gui/eos-slides/recipe.toml` bumped, `scripts/eos-repos.sh pins --strict` green
