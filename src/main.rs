//! E-OS Slides — Presentation editor for E-OS — slides, outline and full-screen show.
//!
...
const USAGE: &str = "\
eos-slides — E-OS Slides

USAGE:
    eos-slides [OPTIONS]

OPTIONS:
    --selftest        run the headless model proof, print EOS-SLIDES-SELFTEST-OK, exit 0
    --check-backend   install the window backend and build the window without
                      entering the event loop, print EOS-SLIDES-BACKEND-OK, exit 0
...
fn main() {
    ...
    if args.iter().any(|a| a == "--selftest") {
        match model::selftest() {
            ...
            Err(err) => {
                println!("EOS-SLIDES-SELFTEST-FAIL: {err}");
                eprintln!("EOS-SLIDES-SELFTEST-FAIL: {err}");
                std::process::exit(1);
            }
        }
        return;
    ...
    if let Some(unknown) = args.iter().find(|a| a.starts_with('-')) {
        eprintln!("eos-slides: unknown option {unknown}\n");
        eprint!("{USAGE}");
        std::process::exit(1);
    }

    if let Err(err) = install_platform() {
        eprintln!("eos-slides: {err}");
        std::process::exit(2);
    }
    ...
    if let Err(err) = win.run() {
        eprintln!("eos-slides: event loop failed: {err}");
        std::process::exit(1);
    }
}
